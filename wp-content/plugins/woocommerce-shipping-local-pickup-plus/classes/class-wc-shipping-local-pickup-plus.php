...
class WC_Shipping_Local_Pickup_Plus extends WC_Shipping_Method {
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( (bool) $discount ) {
			$discount_display = is_float( $discount ) ? wc_price( $discount ) : $discount;
	...
	public function get_discount_data( $data, $code ) {
		...
		if ( is_float( $discount ) && $discount > 0 ) {
			$data = array(
	...
	public function render_pickup_location_selection_row( $package_index ) {
		...
		if ( 1 == count( $this->pickup_locations ) ) {
			...
			if ( 'select' == $this->get_checkout_pickup_location_styling() ) {
				echo '<select name="pickup_location[' . $package_index . ']" class="pickup_location" data-placeholder="' . __( 'Välj en butik för upphämtning', 'woocommerce-shipping-local-pickup-plus' ) . '" style="width: 100%;">';
				echo '<option value="">' . __( 'Välj en butik för upphämtning', 'woocommerce-shipping-local-pickup-plus' ) . ' </option>';
				foreach ( $this->pickup_locations as $location ) {
