...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.4.0', __( 'WooCommerce Local Pickup Plus', 'woocommerce-shipping-local-pickup-plus' ), __FILE__, 'init_woocommerce_shipping_local_pickup_plus', array(
...
class WC_Local_Pickup_Plus extends SV_WC_Plugin {
	 ...
	 *
	 * @TODO Remove this as part of WC 2.7 compat {TZ 2016-05-30}
	 *
	 * @since 1.13.0
	 */
	public function __get( $name ) {
		...
		switch ( $name ) {
			...
			case 'shop_order_cpt':
				...
				_deprecated_function( 'wc_local_pickup_plus()->shop_order_cpt', '1.13.0', 'wc_local_pickup_plus()->get_shop_order_cpt_instance()' );
		...
		if ( is_checkout() ) {
			...
			if ( $local_pickup_plus_is_available ) :
				...
				$( document.body ).on( 'updated_checkout', function() {
					...
					if ( $( 'input[type="hidden"][value="local_pickup_plus"]' ).length > 0 ) {
