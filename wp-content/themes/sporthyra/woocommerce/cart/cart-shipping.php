...
<tr class="shipping">
	...
	<h3><?php echo __("Frakt"); ?></h3>	
		<?php if ( $show_package_details ) : ?>
			<?php echo '<p class="woocommerce-shipping-contents"><small>För:' . esc_html( $package_details ) . '</small></p>'; ?>
		<?php endif; ?>
		...
		<?php elseif ( 1 === count( $available_methods ) ) :  ?>
			<?php
				...
				printf( '%3$s <input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d" value="%2$s" class="rvlvr_bold shipping_method" />', $index, esc_attr( $method->id ), "<b>" . wc_cart_totals_shipping_method_label( $method ) . "</b>" );
				do_action( 'woocommerce_after_shipping_rate', $method, $index );
			?>
